...
const getServerSideProps = async (data) => {
  try {
    ...
    if (query.tab === TABS.TESTS) {
      marks = await testMarksService.getCourseMarks(data.params.id, page, limit);
      logger.info('Test marks are:', marks);
    } else {
      marks = await taskMarksService.getCourseMarks(data.params.id, page, limit);
